...
public class CmdRemoveRoleFromMember implements Command
{
				...
				if (RoleFactory.getRoleByName(args[0]) != null)
				{
					if (args.length >= 2 && args.length <= 2)
					{
						...
						else
						{
							...
							logger.addState(args[1] + IStaticCommand.CMD_REMOVE_ROLE_MEMBER_DONT_EXISTS,
							        this.toString());
						}
					}
					else
	...
	public void action(String[] args, GuildMessageReceivedEvent event)
	{
		...
		try
		{
			gc.removeSingleRoleFromMember(MemberFactory.getMemberByEName(args[1]).getMember(),
			        RoleFactory.getRoleByName(args[0])).queue();
	...
	public void executed(boolean success, GuildMessageReceivedEvent event)
	{
		if (success)
		{
			logger.addLogMessage(ILogCommand.CMD_REMOVE_ROLE_FROM_MEMBER_SUCCESS, ELogMsgType.STATE, this.toString(),
			        ILogCommand.CMD_EXE);
		}
		else
		{
			logger.addLogMessage(ILogCommand.CMD_REMOVE_ROLE_FROM_MEMBER_FAILED, ELogMsgType.STATE, this.toString(),
			        ILogCommand.CMD_EXE);
		}
		writer = null;
		logger = null;
