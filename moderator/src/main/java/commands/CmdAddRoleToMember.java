...
public class CmdAddRoleToMember implements Command
{
	...
	public void action(String[] args, GuildMessageReceivedEvent event)
	{
		...
		try
		{
			gc.addSingleRoleToMember(MemberFactory.getMemberByEName(args[1]).getMember(),
	...
	public void executed(boolean success, GuildMessageReceivedEvent event)
	{
		if (success)
		{
			logger.addLogMessage(ILogCommand.CMD_ADD_ROLE_TO_MEMBER_SUCCESS, ELogMsgType.STATE, this.toString(),
			        ILogCommand.CMD_EXE);
		}
		else
		{
			logger.addLogMessage(ILogCommand.CMD_ADD_ROLE_TO_MEMBER_FAILED, ELogMsgType.STATE, this.toString(),
			        ILogCommand.CMD_EXE);
		}
		logger = null;
		writer = null;
